...
import numpy as np
import os
from utils import mediapipe_detection, get_center_lh, get_center_rh, points_detection, points_detection_hands, draw_landmarks_custom
...
# --- load shield video ---
# --- load shield video ---
effect_files = ['effects/shield.mp4', 'effects/rasengan_shuriken.mp4', 'effects/shield_effect.mp4']
current_effect_index = 0
shield = cv2.VideoCapture(current_directory + '/' + effect_files[current_effect_index])
...
with mp_holistic.Holistic(min_detection_confidence=0.5,
    ...
    if use_virtual_cam:
        ...
        print("📋 Gesture Sequence: KEY_1 → KEY_2 → KEY_3 (activate shields)")
        print("📋 Shield Deactivation: KEY_4")
        print("📋 Toggle Shields: Press 's'")
        print("📋 Toggle Debug Mode: Press 'd'")
        print("📋 Switch Effect: Press 'e'")
        print("📋 Toggle Help: Press 'h'")
        print("⌨️  Press 'q' to quit" + (" (in OpenCV window)" if show_window else "") + " or Ctrl+C")
        print("="*60 + "\n")
    ...
    try:
        while cap.isOpened():
            ...
            if DEBUG:
                print(f"\rFPS: {int(fps)} | {status_shields} | {status_k1} {status_k2} {status_k3}", end="", flush=True)
            else:
            ...
            if DEBUG:
                ...
                cv2.putText(frame, f'Effect: {effect_files[current_effect_index].split("/")[-1]}', (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            ...
            if SHOW_HELP:
                ...
                overlay = frame.copy()
                cv2.rectangle(overlay, (10, height - 160), (250, height - 10), (0, 0, 0), -1)
                cv2.addWeighted(overlay, 0.6, frame, 0.4, 0, frame)
                
                # Help Text
                cv2.putText(frame, "CONTROLS:", (20, height - 135), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                cv2.putText(frame, "'s': Toggle Shields", (20, height - 110), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(frame, "'e': Switch Effect", (20, height - 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(frame, "'d': Toggle Debug", (20, height - 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(frame, "'h': Toggle Help", (20, height - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(frame, "'q': Quit", (20, height - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            ...
            if show_window and key == ord('q'):
                break
            elif show_window and key == ord('s'):
                SHIELDS = not SHIELDS
                KEY_1 = False
                KEY_2 = False
                KEY_3 = False
            elif show_window and key == ord('d'):
                DEBUG = not DEBUG
            elif show_window and key == ord('e'):
                current_effect_index = (current_effect_index + 1) % len(effect_files)
                shield.release()
                shield = cv2.VideoCapture(current_directory + '/' + effect_files[current_effect_index])
            elif show_window and key == ord('h'):
